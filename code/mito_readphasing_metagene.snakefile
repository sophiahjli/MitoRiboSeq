...
rule all:
	input:
		expand("{output_dir}/metagene/{sample}/{sample}_ND6stop_metagene_overview.png",output_dir = output_dir, sample=SAMPLES),
        expand("{output_dir}/metagene/{sample}/{sample}_ND6stop_metagene_profile.txt",output_dir = output_dir, sample=SAMPLES),

        expand("{output_dir}/metagene/{sample}/{sample}_ND4start_metagene_overview.png",output_dir = output_dir, sample=SAMPLES),
        expand("{output_dir}/metagene/{sample}/{sample}_ND4start_metagene_profile.txt",output_dir = output_dir, sample=SAMPLES),
...
rule metagene_stop_ND6:
    ...
    output: 
        profile = "{output_dir}/metagene/{sample}/{sample}_ND6stop_metagene_profile.txt",
        overview = "{output_dir}/metagene/{sample}/{sample}_ND6stop_metagene_overview.png"
    ...
    shell:
        ...
        "{wildcards.output_dir}/metagene/{wildcards.sample}/{wildcards.sample}_ND6stop" \
...
rule metagene_start_ND4:
    ...
    output: 
        profile = "{output_dir}/metagene/{sample}/{sample}_ND4start_metagene_profile.txt",
        overview = "{output_dir}/metagene/{sample}/{sample}_ND4start_metagene_overview.png"
    ...
    shell:
        'metagene count "genome/chrM_orfs_start_rois_ND4.txt" \
        "{wildcards.output_dir}/metagene/{wildcards.sample}/{wildcards.sample}_ND4start" \
        --count_files "{input.counts}" --{config[mapping_function]} \
        --norm_region 30 200 --min_counts 0 --cmap Blues'
